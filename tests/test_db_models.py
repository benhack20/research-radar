...
# 动态添加backend/app到sys.path，避免导入问题
sys.path.append(os.path.abspath(os.path.join(os.path.dirname(__file__), '../backend/app')))
from models import Base, Scholar, Paper, Patent, SyncLog
...
def test_create_paper_and_patent(db_session):
    ...
    paper_data = {
        ...
        "urls": json.dumps(["https://www.alphaxiv.org/abs/2505.11888"]),
        "versions": json.dumps([
        ...
        "year": 2025,
        "update_times": json.dumps({"u_a_t": "2025-07-22T17:43:43.092Z", "u_v_t": "2025-07-22T17:43:43.092Z"})
    }
    ...
    patent_data = {
        ...
        "pct": json.dumps([]),
        "priority": json.dumps([{"num": "11244374"}]),
    ...
    assert json.loads(p.authors)[-1]["name"] == "Chun Yu"
    assert p.year == 2025
    assert p.lang == "en"
    assert p.num_citation == 0
    assert p.pdf.startswith("https://cz5waila03cyo0tux1owpyofgoryroob.aminer.cn/")
    assert json.loads(p.urls)[0].startswith("https://www.alphaxiv.org/")
    assert json.loads(p.versions)[0]["src"] == "alphaxiv"
    assert "u_a_t" in json.loads(p.update_times)
    # 断言专利字段
    pt = db_session.query(Patent).filter_by(aminer_id="67a734db9781ce1a6aa43660").first()
    assert pt is not None
    title_obj = json.loads(pt.title)
    assert "en" in title_obj and "zh" in title_obj
    assert pt.app_date == "2024-09-05T00:00:00Z"
    assert pt.app_num == "202411244374"
    assert json.loads(pt.applicant)[0]["name"] == "清华大学"
    assert json.loads(pt.assignee)[0]["name"].startswith("UNIV TSINGHUA")
    assert pt.country == "CN"
    assert isinstance(json.loads(pt.cpc), list)
    assert json.loads(pt.inventor)[1]["name"] == "喻纯"
    assert json.loads(pt.ipc)[0]["l4"] == "G06F003/01"
    assert json.loads(pt.ipcr)[0]["l4"] == "G06F3/01"
